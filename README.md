...
- #18.4 사진찍기 2<br>
TouchableOpacity를 활용하여 카메라를 앞뒤로 뒤집는 기능을 구현하였다.
...
- #18.8 Uploading to Backend<br>
axios 를 활용하여 백앤드와 REST방식으로 통신하여 데이터를 백엔드에서 받게 하려고 했다.
에러를 만났는데 기존에 안드로이드가 localhost와 통신할수 없어 ngrok이라는 포트포워딩 프로그램을 사용했는데 그것때문에 네트워크 에러가 난 것으로 추측되었고 그래서 axios에 주소를 포트포워딩된 주소로 넣어줫으며 실행될때마다 포트포워딩 주소가 바뀌기 때문에 하나의 폴더에서 그 주소를 관리하도록 만들었다.


- #18.9 업로드 뮤테이션 사용하기<br>
난 노마드와는 다르게 유튜브 영상을 공유하는 SNS를 만들어서 이미지는 제대로 올라가지 않지만 업로드 기능은 제대로 구현을 해 놓았다. Mutation에서 refetch 기능을 사용하여 피드를 자동으로 새로고침하는것까지 구현을 하였따. 이제 배포 시작~!
